...
const BASE_URL =
  "https://formation-68152-default-rtdb.europe-west1.firebasedatabase.app/";
let classesInfos = [];
...
const fetchClassesStats = async (classId) => {
  const response = await fetch(
    BASE_URL + "charClasses/" + classId + "/stats.json"
  ); // https://m2i-formation-ff681-default-rtdb.europe-west1.firebasedatabase.app/charClasses/-NoWoLaxeNix7I3a3kMO.json
  if (!response.status === 200) throw new Error("Something went wrong...");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  charClassSelect.addEventListener("change", async () => {
    console.log(charClassSelect.value);
    ...
    if (classesInfos.find((x) => x.id === value)) {
      ...
      for (const stat in stats) {
        ...
        const inputElement = document.querySelector(
          `input[id="char-stats-${stat}"]`
        );
        console.log(inputElement);
        inputElement.value = 20 + stats[stat];
        actualStats[stat] = 20 + stats[stat];
      }
    ...
    input.addEventListener("change", () => {
      ...
      if (input.value > actualStats[changingKey]) {
        ...
        if (
          ...
          difference < +availablePointsSpan.textContent
        ) {
          availablePointsSpan.textContent =
            +availablePointsSpan.textContent - difference;
